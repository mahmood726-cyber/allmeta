...
NMA truth-recovery modality from scratch.

    python run_all.py            # full benchmark (500 reps, Bayesian 120; ~5 min on 4 cores)
    python run_all.py --smoke    # fast end-to-end check (100 reps; ~15-30 s)
    python run_all.py --no-sim   # skip the harness; rebuild report/figures/paper
                                 # from the committed results JSON (seconds)

Stages, in order:
  1. harness_nma.py    -> results_nma_{profile}.json    (seeded simulation)
  2. partialid_nma.py  -> results_partialid.json         (partial-ID experiment)
  3. report_nma.py     -> REPORT.md                      (tabular summary)
  ...
  7. pytest tests/     -> correctness gate

Determinism: base seed 20260615; same --reps reproduce every number regardless of
--procs. The full profile reproduces the committed results_nma_full.json exactly.
"""
import argparse
import os
import subprocess
...
def main():
    ...
    ap.add_argument("--smoke", action="store_true", help="fast 100-rep check")
    ...
    reps = 100 if args.smoke else 500
    bayes_reps = 30 if args.smoke else 120

    if not args.no_sim:
        run([PY, "harness_nma.py", "--profile", profile,
             "--reps", str(reps), "--bayes-reps", str(bayes_reps),
             "--bayes-blocks", "coverage", "--procs", str(args.procs)])
        run([PY, "partialid_nma.py"])
        if not args.smoke:
            run([PY, "report_nma.py"])
