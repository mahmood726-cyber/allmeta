...
// V10-07: PI undefined for k<3 per advanced-stats.md / Higgins 2009.
// Return NaN with explicit warning instead of silently flooring df at 1.
if(a<3){return{lower:NaN,upper:NaN,se:NaN,df:0,df_method:i,df_note:"PI undefined for k<3 (Higgins 2009; advanced-stats.md)",t_critical:NaN,method:"HTS",warning:"k<3 — PI not defined"}}
let r,o;
switch(i){
case "k-1":
...
// V11 F-02: PI undefined for k<3 (advanced-stats.md / Higgins 2009).
if(s<3){return{lower:NaN,upper:NaN,se:NaN,df:0,t_critical:NaN,method:"Noma",warning:"k<3 — PI not defined"}}
...
// Stubbed: minified Blob worker source was truncated upstream.
// See review-findings — track restoration via the standalone GOSH app.
function runGOSHAnalysis(){
const e=AppState.results;
if(!e||!e.studies){showToast("Run analysis first","error");return}
showToast("GOSH analysis is temporarily unavailable in this build — use the standalone GOSH app for now.","warn")
}
...
const mu_hdi_lower=quantile(muSorted,0.025);
...
if(btn){
btn.disabled=true;
btn.innerHTML='<span class="btn-spinner">⟳</span>Running MCMC(0%)...';
}
...
if(btn){
btn.innerHTML=`<span class="btn-spinner">⟳</span>Chain ${chain}/${totalChains}(${progress.toFixed(0)}%)...`;
}
if(statusDiv){
...
${convergence.warning ? `<p style="color:var(--color-warning-500);margin-top:var(--space-2);font-size:var(--text-xs);">⚠️ ${convergence.warning}</p>`:
`<p style="color:var(--color-success-500);margin-top:var(--space-2);font-size:var(--text-xs);">✓ Chains converged(R-hat<1.1)</p>`}
</div>
<div style="margin-bottom:var(--space-3);">
...
<tr style="border-bottom:1px solid var(--border-subtle);">
<td style="padding:var(--space-1);">Effect(μ)</td>
...
<tr style="border-bottom:1px solid var(--border-subtle);">
<td style="padding:var(--space-1);">τ²(heterogeneity)</td>
...
<tr style="border-bottom:1px solid var(--border-subtle);">
<td style="padding:var(--space-1);">τ(SD)</td>
...
<td style="text-align:right;padding:var(--space-1);">—</td>
</tr>
<tr style="border-bottom:1px solid var(--border-subtle);">
<td style="padding:var(--space-1);">I²(%)</td>
...
yaxis:{title:'μ(Effect)',domain:[0.55,1]},
xaxis2:{title:'Iteration',domain:[0,1],anchor:'y2'},
yaxis2:{title:'τ²',domain:[0,0.45],anchor:'x2'},
paper_bgcolor:'rgba(0,0,0,0)',
plot_bgcolor:'rgba(0,0,0,0)',
...
t&&(t.disabled=!1);
showToast("RoBMA is temporarily unavailable in this build — use the WebR Validator or local R for RoBMA.","warn")
}
...
name:s.name,
rate_c:s.events_c/s.n_c,
rate_t:s.events_t/s.n_t,
weight:s.n_t+s.n_c,
n_t:s.n_t,
n_c:s.n_c,
events_t:s.events_t,
...
const rankV=getRanks(sei.map(s=>s*s));
...
const selectionProb=pval<0.05 ? 1:0.3;
...
bestModel:results[0],
reference:'Bartoš et al.(2023)Research Synthesis Methods'
};
}
...
reference:'Bartoš&Schimmack(2025)'
};
}
...
reference:'Voracek et al.(2019)Zeitschrift für Psychologie'
};
}
...
note:'Improved PI accounts for tau² estimation uncertainty'
},
...
const delta=options.expectedEffect||0.3;
const cumulative=[];
let cumulativeZ=0;
let cumulativeVar=0;
...
const futilityBoundary=delta*Math.sqrt(currentInfo)*0.5;
...
reference:'López-López et al.(2014)'
};
}
...
return{
method:`Outlier-Robust tau²(${method})`,
...
if(p<0.025)return w.sig;
if(p<0.10)return w.marginal;
return w.nonsig;
});
...
const sei=vi.map(v=>Math.sqrt(v));
const z=yi.map((y,i)=>y/sei[i]);
const pvals=z.map(zz=>1-pnorm(zz));
...
const beta1=sumWXY/sumWX2;
const beta0=meanY-beta1*meanPrec;
...
interpretation:`Treatment worthwhile when baseline risk ≥ ${(risk*100).toFixed(1)}%`
};
break;
}
}
...
reference:'Infanger&Schmidt-Trucksäss(2019)'
};
}
...
const method=options.method||'ML';
const nQuad=options.nQuad||13;
const n1=ai.map((a,i)=>a+bi[i]);
const n2=ci.map((c,i)=>c+di[i]);
const p1=ai.map((a,i)=>a/n1[i]);
const p2=ci.map((c,i)=>c/n2[i]);
const hasZeroCells=ai.some((a,i)=>a===0||bi[i]===0||ci[i]===0||di[i]===0);
const ghNodes=gaussHermiteNodes(nQuad);
function logLikelihood(mu,tau){
let ll=0;
for(let i=0;i<k;i++){
let integral=0;
for(let q=0;q<nQuad;q++){
const theta_i=mu+Math.sqrt(2)*tau*ghNodes.nodes[q];
let logL_i;
if(measure==='OR'){
const odds1=Math.exp(theta_i/2);
const odds2=Math.exp(-theta_i/2);
const prob1=odds1/(1+odds1);
const prob2=odds2/(1+odds2);
logL_i=ai[i]*Math.log(prob1+1e-10)+bi[i]*Math.log(1-prob1+1e-10)+
ci[i]*Math.log(prob2+1e-10)+di[i]*Math.log(1-prob2+1e-10);
}else{
const p1_i=1/(1+Math.exp(-theta_i));
const p2_i=0.5;
logL_i=ai[i]*Math.log(p1_i+1e-10)+bi[i]*Math.log(1-p1_i+1e-10);
}
integral+=ghNodes.weights[q]*Math.exp(logL_i);
}
ll+=Math.log(integral/Math.sqrt(Math.PI)+1e-300);
}
return ll;
}
let bestMu=0,bestTau=0.1,bestLL=-Infinity;
for(let mu=-3;mu<=3;mu+=0.2){
for(let tau=0.01;tau<=2;tau+=0.1){
const ll=logLikelihood(mu,tau);
if(ll>bestLL){
bestLL=ll;
bestMu=mu;
bestTau=tau;
...
const muRange=[bestMu-0.3,bestMu+0.3];
const tauRange=[Math.max(0.001,bestTau-0.15),bestTau+0.15];
for(let mu=muRange[0];mu<=muRange[1];mu+=0.05){
for(let tau=tauRange[0];tau<=tauRange[1];tau+=0.02){
const ll=logLikelihood(mu,tau);
if(ll>bestLL){
bestLL=ll;
bestMu=mu;
...
const h=0.001;
const d2ll_dmu2=(logLikelihood(bestMu+h,bestTau)-2*logLikelihood(bestMu,bestTau)+
logLikelihood(bestMu-h,bestTau))/(h*h);
const seMu=Math.sqrt(-1/d2ll_dmu2);
const estimate=measure==='OR' ? Math.exp(bestMu):bestMu;
const ci_lower=measure==='OR' ? Math.exp(bestMu-1.96*seMu):bestMu-1.96*seMu;
const ci_upper=measure==='OR' ? Math.exp(bestMu+1.96*seMu):bestMu+1.96*seMu;
return{
method:'Generalized Linear Mixed Model(GLMM)',
model:'Binomial-Normal',
measure,
k,
estimate:{
log:bestMu,
...
hasZeroCells,
zeroCellHandling:'Exact likelihood-no continuity correction needed',
quadraturePoints:nQuad,
interpretation:hasZeroCells
? 'GLMM recommended for rare events with zero cells'
:'GLMM provides exact inference without normal approximation',
...
const tables={
7:{
nodes:[-2.651961,-1.673552,-0.816288,0,0.816288,1.673552,2.651961],
weights:[0.000972,0.054516,0.425607,0.810264,0.425607,0.054516,0.000972]
},
13:{
nodes:[-3.839,-3.029,-2.351,-1.756,-1.219,-0.656,0,0.656,1.219,1.756,2.351,3.029,3.839],
weights:[0.000001,0.000091,0.002547,0.032587,0.206516,0.649054,0.953089,
0.649054,0.206516,0.032587,0.002547,0.000091,0.000001]
},
21:{
nodes:[-5.078,-4.288,-3.6,-2.976,-2.395,-1.844,-1.314,-0.799,-0.293,0.209,0.707,
1.201,1.691,2.178,2.661,3.142,3.622,4.102,4.584,5.071,5.564],
weights:new Array(21).fill(1/21)
}
};
return tables[n]||tables[13];
}
...
nullHypothesis:'tau²=0(homogeneity)',
LR_statistic:LR,
df:'0:1 mixture',
pValue,
tau2_ML:tau2,
...
const studyData=AppState.data&&AppState.data.length>0
? AppState.data
