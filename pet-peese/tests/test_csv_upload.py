"""CSV upload wiring tests for pet-peese retrofit."""
import re
...
def test_csv_upload_columns_match_engine():
    # PET-PEESE input is study/yi/sei (SE direct, not variance vi)
    html = INDEX.read_text(encoding="utf-8")
    assert re.search(r"name:\s*['\"]study['\"]", html), "study column missing from csv-upload config"
    assert re.search(r"name:\s*['\"]yi['\"]", html), "yi column missing from csv-upload config"
    assert re.search(r"name:\s*['\"]sei['\"]", html), "sei column missing (pet-peese uses SE not variance)"
