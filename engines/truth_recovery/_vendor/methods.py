...
def _puni_profile(mu, y, sig, sigma, ycv):
    """Profile out tau2>=0 at fixed mu. Returns (tau2_hat, nll_min)."""
    f = lambda lt2: _puni_negll(mu, np.exp(lt2), y, sig, sigma, ycv)
    r = optimize.minimize_scalar(f, bounds=(np.log(1e-8), np.log(1e3)),
...
def p_uniform_star(y, v):
    ...
    mu0 = _fe_mu(y, v)
    t0 = max(1e-6, _reml_tau2(y, v))
    obj = lambda p: _puni_negll(p[0], np.exp(p[1]), y, sig, sigma, ycv)
    try:
        res = optimize.minimize(obj, [mu0, np.log(t0)], method="Nelder-Mead",
    ...
    tau2_hat, nll_min = _puni_profile(mu_hat, y, sig, sigma, ycv)

    # Profile-likelihood CI: solve 2*(nll(mu) - nll_min) = qchisq(.95,1).
    target = nll_min + 0.5 * _CHI2_1_95
    prof = lambda m: _puni_profile(m, y, sig, sigma, ycv)[1]
    ...
    ci_lo = _root(-1.0)
    ci_hi = _root(+1.0)
    ok = np.isfinite(ci_lo) and np.isfinite(ci_hi)
    if ok:
    ...
    return {"method": "p-uniform*", "mu": mu_hat, "se": se,
            "ci_lo": ci_lo, "ci_hi": ci_hi, "tau2": tau2_hat,
