...
- [x] Sentinel-equivalent lint pass on every PR — **done 2026-06-03**: `scripts/lint_repo.py` (pure stdlib) enforces shipped-asset invariants across all 91 catalog entry points + authored `shared/` (no external CDN `<script src>`/`<link href>`, no hardcoded local paths, no BOM, no unpopulated `{{…}}`/`REPLACE_ME` placeholders), wired via `.github/workflows/lint.yml` on push + PR. Blocks NEW violations; a documented ALLOWLIST accepts the known baseline. Baseline cleanup: stripped a BOM from `nma-pro-v8.0.html`; allowlisted Google-Fonts loads in the two non-evidence utility apps (`focus-studio`, `kanban-lab`) pending vendoring. Div-balance is intentionally excluded (JS-string false positives — stays a manual post-edit check per AGENTS.md).
  - **Follow-up (P2):** vendor or system-font the `focus-studio` / `kanban-lab` Google Fonts to remove the last two CDN dependencies and tighten their CSP back to `'self'`.
