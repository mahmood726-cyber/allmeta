...
- [~] TruthCert receipt extension — shared UI shipped (`shared/truthcert-ui.js`: key panel + receipt download, fails closed; spec `hub/shared/tests/truthcert-ui.spec.mjs`). Wired into `workbench`, `forest-plot`, `funnel-plot`, `heterogeneity` (4/~17). Remaining bus-writers are a one-call-each sweep (`attachReceiptButton` + 2 buttons): meta-regression, bayesian-ma/mcmc, cumulative-subgroup, tsa, influence, gosh, gosh-metareg, pet-peese, pubbias-tests, copas, limit-ma, mh-peto, proportion-ma
