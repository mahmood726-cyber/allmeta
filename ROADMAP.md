...
- [x] Wire import into pooling apps — **complete for every app where importing a {label,est,se} study list is methodologically valid** (20+ readers, incl. `grade-sof`/`webr-validator` as pure consumers, and `cross-design`/`bma-tau-priors` wired 2026-06-03). The 3 apps that load `ma-studies-v1.js` but do NOT read it are by-design not-applicable (assessed 2026-06-03, mirroring the export-side finding): `hsroc` takes DTA 2×2 counts (TP/FP/FN/TN), `multilevel-ma` needs a cluster structure (independent studies degenerate the 3-level model), `p-curve` needs focal-test p-values (not pooled est/se). Not gaps.
...
- [x] Multi-language method help (Spanish, Arabic, Mandarin) — **done 2026-06-04**: the shared glossary-tooltip system (`hub/shared/tooltips.js` + `glossary.json`, 92 unique stats terms surfaced via `abbr[data-gloss]`) is now language-aware. Added `glossary.{es,ar,zh}.json` (full key-parity machine translations; proper nouns/acronyms/formulas kept verbatim, expansions translated). `tooltips.js` resolves `glossary.<lang>.json` from a persisted `localStorage['alm-lang']`, self-injects a compact 4-language switcher **only on pages that have `data-gloss` terms** (zero per-app HTML edits — no codemod), sets `dir=rtl` for Arabic, and **falls back to the authoritative English file** if a translation is missing. Non-English tips carry a per-language `_meta.disclaimer` (**"machine-translated, pending expert review"** — English stays authoritative; this is the honest-provenance contract agreed with the maintainer, not a silent overclaim). Spec `hub/shared/tests/tooltips.spec.mjs` extended with 5 i18n tests (switcher injection, ES load+disclaimer, AR RTL, stored-lang-on-load, English fallback) — 7/7 green on Edge; a11y sweep 76/76 unchanged (switcher introduces no axe regression). **Deferred (P3):** native/expert review of the AR/ZH technical terminology to lift the disclaimer, and translating per-app prose (only the shared glossary is translated today, which is the cross-app method-help surface).
...
<!-- sentinel:skip-line P1-unpopulated-placeholder  (line below quotes the placeholder tokens the lint rule forbids; it documents the rule, it is not an unpopulated token) -->
- [x] Sentinel-equivalent lint pass on every PR — **done 2026-06-03**: `scripts/lint_repo.py` (pure stdlib) enforces shipped-asset invariants across all 91 catalog entry points + authored `shared/` (no external CDN `<script src>`/`<link href>`, no hardcoded local paths, no BOM, no unpopulated `{{…}}`/`REPLACE_ME` placeholders), wired via `.github/workflows/lint.yml` on push + PR. Blocks NEW violations; a documented ALLOWLIST accepts the known baseline. Baseline cleanup: stripped a BOM from `nma-pro-v8.0.html`; **vendored** the Google Fonts of the two non-evidence utility apps (`focus-studio`, `kanban-lab`) into `shared/fonts/` (latin-subset woff2 + local `@font-face` CSS) and tightened their CSP back to `'self'` — so the ALLOWLIST is now **empty** and every catalog app is genuinely CDN-free (156 shipped assets clean, 0 allowlisted; verified in-browser: `*-fonts.css` + woff2 load 200, no CSP violation). Div-balance is intentionally excluded (JS-string false positives — stays a manual post-edit check per AGENTS.md).
- [x] Quarterly review cycle (v12, v13, …) keep the deferred list small — **operationalised 2026-06-03**: `REVIEW-CYCLE.md` documents the cadence + 4-persona protocol; `scripts/review_cycle.py` generates an evidence-based prep snapshot (live catalog/parity/lint health + the deferred list parsed straight from this file's open boxes, so reviews never run on memory); `.github/workflows/quarterly-review.yml` runs it on a quarterly cron + manual dispatch and uploads the snapshot artifact (non-intrusive — no auto-issues/commits). Each cycle's job is to triage the deferred list **down** and record findings in `review-findings-v<N>.md`.
