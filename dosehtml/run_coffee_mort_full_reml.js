...
async function run() {
  const csv = fs.readFileSync(path.resolve('C:/HTML apps/dosehtml/coffee_mort_full_shifted.csv'), 'utf8').trim();
  ...
  const hasRun = await page.evaluate(() => typeof runAnalysis === 'function');
  console.log('runAnalysis available:', hasRun);
  ...
  try {
    ...
    const state = await page.evaluate(() => ({
      hasResults: typeof analysisResults !== 'undefined' && !!analysisResults,
      resultsKeys: typeof analysisResults !== 'undefined' && analysisResults ? Object.keys(analysisResults) : []
    }));
    console.error('Wait for analysisResults timed out.', state);
    throw err;
  }
  ...
  fs.writeFileSync(path.resolve('C:/Users/user/dosehtml_generated_coffee_full_reml.R'), rCode, 'utf8');
