...
- **Classifier (M-5 closed):** **both reviewers' labels** now train the model (was
  R1-only), **unigram+bigram+MeSH** features, class-weighted logistic regression, a
  **5-fold cross-validated AUC** and a data-driven **stopping signal** surfaced in-UI.
...
|---|---|
| Active-learning recall "not benchmarked" | **WSS@95 = 0.67** on Cohen ACE-Inhibitors (screen 28 % of 2,544 records to find 95 % of relevant); recall@20 % = 0.85 |
...
- **vs Rayyan:** wins on cost, privacy, reproducibility, transparency, and now
  **100k-record scale** and **measured WSS@95 = 0.67** active learning; matches on
...
- **vs ASReview:** competitive active-learning recall **with the same kind of labelled
  benchmark ASReview uses**, while remaining a zero-install single-file browser app.
