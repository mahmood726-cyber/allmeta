...
| C-15 | PMC11443784 `Fig16` vs `Fig13/14/15` | Axis annotation **reversed** within one paper: Fig16 prints *Favors short regimens* LEFT / *Favors standard longer* RIGHT; Figs 13–15 print the opposite — same template, same direction of comparison, same RR-of-adverse-event outcome. One orientation is mislabelled. Any pipeline inferring direction-of-benefit from figure position rather than the printed axis inverts one of them. |
| C-16 | PMC11201327 `Fig16` | Caption says "association between **sex** and undernutrition"; the plot's own group headers are **EPTB / PTB**. Checked for a systematic caption offset — there is none: Fig17/18/19 all match their captions. So either the caption or the figure numbering is wrong in the published article. |
| C-17 | PMC11201327 `Fig2` | Last study's year printed as **`206`** (three digits), confirmed at 6x — not a read error. List is year-descending and the row sits below 2014, so `2006` was likely meant. Recorded verbatim, not corrected. |
| C-18 | PMC11201327 `Fig18` | Right-hand group header prints **"Uneployed"**, confirmed on a 5x crop to exclude a misread. Transcribed verbatim. |
